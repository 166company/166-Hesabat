  ...
  async function loadStatus() {
    try {
      ...
      if (res.data.customers?.length > 0)
        setSelectedCustomerIds(new Set(res.data.customers.map((c: GoogleCustomer) => c.id)));
    } catch { }
  }
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold text-[#4285F4]">{t('google.title')}</h2>
        <div className="flex gap-2">
          {customerReports.length > 0 && <ExportButton data={exportData} filename="google-ads-report" accentColor="#4285F4" />}
          {status?.connected && (
            <button onClick={disconnect} className="px-3 py-2 text-xs text-red-600 border border-red-200 rounded-lg hover:bg-red-50">
              {t('google.disconnect')}
            </button>
          )}
        </div>
      ...
      {!status?.connected && step !== 'verify' && (
        <div className="bg-[#fef9e7] border border-[#FBBC04]/30 rounded-xl p-8 text-center">
          <div className="text-4xl mb-3">🔍</div>
          <h3 className="font-semibold text-gray-800 mb-2">{t('google.notConnected')}</h3>
          <p className="text-xs text-gray-500 mb-6 max-w-sm mx-auto">Google Ads hesabınızı bağlamaq üçün Google OAuth ilə giriş edin.</p>
          <button onClick={connectGoogle} disabled={connectLoading}
            className="px-6 py-2.5 text-sm font-medium text-white rounded-lg bg-[#4285F4] hover:bg-[#3367d6] disabled:opacity-50 transition-colors">
      ...
      {status?.connected && status.verified && (
        <>
          ...
          {/* Date Range */}
          <div className="mb-6">
            <DateRangePicker value={dateRange} onChange={setDateRange} onApply={fetchReport} accentColor="#4285F4" />
          </div>
